...
class Tone_Analysis():
    def __init__(self, text_required):
        ...
        self.data = open('tone.json', 'rb').read()
        self.tone_text = " "
        self.sentence_tone = ""
        self.tone_sentences = ""
    ...
    def configure_json(self, replacement_text, text_toBe_replaced):   
        with fileinput.FileInput("tone.json", inplace=True, backup='.bak') as file:
            for line in file:
    ...
    def analyze_tone_watson(self):
        response = requests.post('https://gateway.watsonplatform.net/tone-analyzer/api/v3/tone', headers=self.headers, params=self.params, data=self.data, auth=('apikey', 'QopQvaHbcHVkTGQB-eqVvuA_vUzKmwfuL9iBnM5CvG2m'))
        print(response.text)
        return response
