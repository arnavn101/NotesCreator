...
class Summarizer:
    ...
    def create_sentence_vectors(self):
        ...
        return (vectorizer.fit_transform(self.formatted_article_text)).toarray()
        ...
        try:
            for x in range(len(self.basic_sentences)):
                for y in range(len(self.basic_sentences)):
                    if x != y:
                        similarity_matrix[x][y] = self.sentence_similarity_calculator(sentence_vectors[x][0],
                                                                                      sentence_vectors[y][0])
        except IndexError:
            pass
    ...
    def page_rank_algorithm(self, similarity_matrix, number_iterations=100, damping_factor=0.85):
        ...
        for x in range(number_iterations):
            new_probability = (damping_factor * similarity_matrix.dot(probability)) + addition_vector
