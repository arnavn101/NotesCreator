...
class PreProcessor():
    ...
    def convert_audio(self):
        if not os.path.isfile("audio.wav"):
            for filename in self.retrieve_files("downloads"):
                file_path = os.path.join("downloads", filename)
                if "Audio" in magic.from_file(file_path) or "audio" in magic.from_file(file_path) \
                        or magic.from_file(file_path) == "data" or "MPEG" in magic.from_file(file_path):
